...
patch(PosStore.prototype, {
    ...
    async editLots(product, packLotLinesToEdit) {
        ...
        const existingLotsName = existingLots.map((l) => l.name + "\u00A0".repeat(160) + l.currency_symbol + l.sale_price);
        ...
        if (payload) {
            ...
            const newPackLotLines = payload
                .filter((item) => !item.id)
                .map((item) => ({ lot_name: item.text.slice(0,5) }));
    ...
    async addLineToOrder(vals, order, opts = {}, configure = true) {
        ...
        const lot_result = await rpc("/web/dataset/call_kw", {
            ...
            args: [[],values.pack_lot_ids],
            kwargs: {},
        });
        
        if (lot_result){
            values.pack_lot_ids[0][1]['lot_name'] = lot_result['lot_name']
            values.price_unit = lot_result['sale_price']
        }
